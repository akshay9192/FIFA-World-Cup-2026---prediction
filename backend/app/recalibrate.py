    ...
    def run(self, db: Session) -> dict[str, Any]:
        ...
        for match in completed:
            ...
            if not team_a or not team_b:
                continue
            target = 1 if match.actual_winner == "Draw" else 2 if match.actual_winner == match.team_a else 0
            rows.append({"features": self.predictor.build_feature_vector(team_a, team_b, {"stage": match.stage}), "target": target})
        train_result = self.predictor.train(rows)
        after = train_result.get("accuracy", before)
        db.add(RecalibrationLog(matches_used=len(rows), model_accuracy_before=before, model_accuracy_after=after, notes="xgboost retrained"))
        db.commit()
        return {"status": "recalibrated", "matches_used": len(rows), "accuracy_before": before, "accuracy_after": after}
