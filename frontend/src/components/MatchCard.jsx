...
export default function MatchCard({ match, compact = false, sequence }) {
  ...
  return (
    <article className={`match-strip ${compact ? 'is-compact' : ''}`} aria-label={`${match.team_a} versus ${match.team_b}`}>
      ...
      {!compact && (
        <div className="probabilities">
          {probabilities.map(([label, value, kind]) => <div key={kind} className={kind}><span>{label}</span><i style={{ '--width': formatPercent(value) }} /><b>{formatPercent(value)}</b></div>)}
